...
export default function Header({ color, bgColor, visible, logo }) {
  ...
  return (
    <>
      <header className={`transition-top fixed w-full ${bgColor} ${visible ? 'top-0' : '-top-52'} duration-300 ease-in-out z-[999999999999]`}>
        <nav className={`px-4 lg:px-6 py-2.5 ${mobileMenuOpen ? 'bg-white' : ''}`}>
          <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
            ...
            <button
              ...
              className="inline-flex items-center p-2 ml-1 text-sm text-gray-200 rounded-lg lg:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-300 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
                    ...
                    {item.dropdown ? (
                      <div className="relative">
                        <button
                          ...
                          lg:p-0 
                          text-red-500 flex ${servicesDropdownOpen ? 'text-red-500 dark:text-red-500' : ''}`}
                      ...
                      <Link to={item.link}>
                        <a
                          className={`block py-2 pr-4 pl-3 font-bold lg:p-0  ${mobileMenuOpen ? 'text-gray-700' : ''} ${activeMenu === index ? 'text-red-500' : color}`}
